class ClanDecorator < Draper::Decorator
  ...
  # Clan index actions
  def actions
    concat(link_to 'Zobacz', object, class: "btn btn-do-ghostie") if current_page?(action: 'index')
  ...
  def approval
    if has_role?(:admin)
      content_tag(:div, class:'clan_actions p-2 text-center mt-4') do
        if object.approval
          link_to (object.approval ? "Odrzuć" : "Zatwierdź"), decline_clan_path(object) , class: 'btn btn-do-ghostie'
        else
          link_to (object.approval ? "Odrzuć" : "Zatwierdź"), approve_clan_path(object) , class: 'btn btn-do-ghostie'
        end
      end
    end
  end
...
#
  ...
  def link_to_clan_list
    link_to "Lista klanów", clans_path, class: "btn btn-do-ghostie"
  end
